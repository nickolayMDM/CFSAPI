...
const getPostsCountUseCaseTest = (
    {
        test,
        validators,
        database,
    ...
    test.describe("Get posts count use case test", () => {
        const getPostsCount = getPostsCountFactory({
            validators,
            database,
            RequestError
        });
        let userEntity, postEntity;
        ...
        test.it("should get the post count", async () => {
            const postsCount = await getPostsCount({
                userID: userEntity.getID()
            });

            test.equal(postsCount, 1, "did not find the accurate posts count");
        });

        test.it("should throw an error without the user ID", async () => {
            await test.rejects(getPostsCount(), RequestError, "Did not receive a request error");
        });
        test.it("should throw an error with an incorrect user ID", async () => {
            await test.rejects(getPostsCount({
                userID: "Bob"
...
module.exports = getPostsCountUseCaseTest;