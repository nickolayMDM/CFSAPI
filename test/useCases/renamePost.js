...
const renamePostFactory = require("../../useCases/renamePost");

const renamePostUseCaseTest = (
    {
        test,
        validators,
        database,
    ...
    test.describe("Rename post use case Test", () => {
        const renamePost = renamePostFactory({
            validators,
            database,
            objectHelpers,
            RequestError
        ...
        test.it("should rename the post", async () => {
            const newName = "Bob's post renamed";
            await renamePost({
                name: newName,
                userID: user.getID(),
                postID: post.getID()
            ...
            test.equal(newPostData.name, newName, "Did not find correct data");
        });

        test.it("should throw an error without a new name", async () => {
            await test.rejects(renamePost({
                userID: user.getID(),
                postID: post.getID()
        ...
        test.it("should throw an error without a user ID", async () => {
            await test.rejects(renamePost({
                name: "Bob's post renamed with error",
                postID: post.getID()
            }), RequestError, "Did not find correct data");
        });
...
module.exports = renamePostUseCaseTest;