...
const getSearchedContentUseCaseTest = (
    {
        test,
        validators,
        database,
    ...
    test.describe("Get searched content use case test", () => {
        const getSearchedContent = getSearchedContentFactory({
            validators,
            database,
            RequestError
        });
        let userEntity, postEntity, folderEntity;
        ...
        test.it("should get the existing content with exact search query", async () => {
            const searchedContent = await getSearchedContent({
                userID: userEntity.getID(),
                searchQuery: "Bob"
            });

            test.equal((searchedContent.posts.length === 1 && searchedContent.folders.length === 1), true, "did not find the content");
        });
        test.it("should get the existing content with case-insensitive search query", async () => {
            const searchedContent = await getSearchedContent({
                userID: userEntity.getID(),
                searchQuery: "bob"
            });
        ...
        test.it("should get nothing with a wrong search query", async () => {
            const searchedContent = await getSearchedContent({
                userID: userEntity.getID(),
                searchQuery: "Garry"
            });

            test.equal((searchedContent.posts.length === 0 && searchedContent.folders.length === 0), true, "found wrong content");
        });

        test.it("should throw an error without the user ID", async () => {
            await test.rejects(getSearchedContent({
                searchQuery: "Bob"
        ...
        test.it("should throw an error with an non-existing user ID", async () => {
            await test.rejects(getSearchedContent({
        ...
        test.it("should throw an error without a search query", async () => {
            await test.rejects(getSearchedContent({
                userID: userEntity.getID()
            }), RequestError, "Did not receive a request error");
        });
        test.it("should throw an error with an incorrect search query", async () => {
            await test.rejects(getSearchedContent({
                userID: userEntity.getID(),
                searchQuery: 42
            }), RequestError, "Did not receive a request error");
        });
        test.it("should throw an error with a too short search query", async () => {
            await test.rejects(getSearchedContent({
                userID: userEntity.getID(),
                searchQuery: "Bo"
            }), RequestError, "Did not receive a request error");
...
module.exports = getSearchedContentUseCaseTest;